      ...
      {isVisible && (
        <div className="fixed inset-0 pointer-events-none z-50">
          {particles.map((particle) => (
            <motion.div
              key={particle.id}
              className="absolute bg-gradient-to-r from-blue-500/30 to-red-500/30 rounded-full"
              style={{
                width: particle.size,
          ...
          <motion.div
            ref={cursorRef}
            className="absolute w-4 h-4 bg-gradient-to-r from-orange-500 to-red-500 rounded-full"
            animate={{
              x: position.x - 8,
              y: position.y - 8,
              scale: 1,
